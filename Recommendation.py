...
class movie: #Creamos la clase película con sus respectivos atributos
...
l_movies = [] #Creamos una lista para almacenar las películas
...
for index, row in df_movies.iterrows():
    ...
    g_movies.add_node(pelicula) #Agregamos la película al grafo en forma de nodo
    l_movies.append(pelicula) #Agregamos la película a la lista.
...
for i in range(len(l_movies)):
    ...
    for j in range(i+1, len(l_movies)):
        ...
        weight = 0
        #Si se detectan similitudes en los atributos de las 2 peliculas que analizamos, se aumenta el peso de la arista que posteriormente las unirá
        ...
        #Creamos la arista dependiendo de la relación entre las películas
        if weight == 1:
...
def obtener_recomendaciones(pelicula, num_recomendaciones):
    ...
    # Obtenemos los vecinos de la película
    vecinos = g_movies.neighbors(pelicula)
    
    # Calculamos los pesos es decir, la similitud de los vecinos
    pesos_similitud = {}
    for vecino in vecinos:
        peso = g_movies[pelicula][vecino]['weight']
        pesos_similitud[vecino] = peso
    
    # Ordenamos los vecinos por peso [de mayor a menor]
    vecinos_ordenados = sorted(pesos_similitud, key=pesos_similitud.get, reverse=True)
    
    # Obtenemos las recomendaciones basadas en los vecinos ordenados
